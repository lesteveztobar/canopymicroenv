#!/bin/bash
# hpc_array.sh — submit one job per site to the Marvin cluster
# Usage: sbatch hpc_array.sh
...
N_MONTHS=${1:-1}  # e.g. sbatch hpc_array.sh 12

for i in "${!SITES[@]}"; do
  sbatch /home/$USER/canopymicroenv/scripts/hpc_job.sh "${SITES[$i]}" "$N_MONTHS"
done
