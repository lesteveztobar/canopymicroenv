...
#
# Usage:
#   sh scripts/run_pipeline.sh                                  # interactive prompt
#   sh scripts/run_pipeline.sh microenv params niche experiments plots
#   sh scripts/run_pipeline.sh params, niche, plots             # commas ok too
#   sh scripts/run_pipeline.sh -y params niche plots            # skip confirmation
#
  ...
  case "$step" in
    ...
    microenv)
      ...
      for site in "${SITES[@]}"; do
        id=$(sbatch --parsable "${DEP_ARG[@]}" scripts/run_microenv.sh "$site" "$N_MONTHS" "$HEIGHT_STEP")
    ...
    params)
      id=$(sbatch --parsable "${DEP_ARG[@]}" --partition=intelsr_short \
             ...
             --wrap="module purge; module load GCCcore/13.3.0 R/4.4.2-gfbf-2024a; Rscript scripts/complex_model/make_params.R")
      echo "  params -> job $id"
      NEW_IDS+=("$id")
      ;;

    niche)
      id=$(sbatch --parsable "${DEP_ARG[@]}" scripts/complex_model/characterize_niches.sh "$HEIGHT_STEP")
      echo "  niche -> job $id"
      NEW_IDS+=("$id")
      ;;
    ...
    experiments)
      ...
      for i in "${!SITES[@]}"; do
        for k in "${!EXP[@]}"; do
          id=$(sbatch --parsable --dependency="afterok:$pid" scripts/complex_model/run_colonization.sh \
    ...
    plots)
      id=$(sbatch --parsable "${DEP_ARG[@]}" scripts/complex_model/run_plots.sh)
      echo "  plots -> job $id"
      NEW_IDS+=("$id")
      ;;
  esac
