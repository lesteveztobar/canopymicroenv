...
def _parse_datetime(name_str):
    ...
    # Spanish, "a las" variant: "31 de mayo de 2026 a las 09:52"
    m = re.match(
...
# ── LABELED FORMAT PARSER (LaElenita / MindoMirador / Saloya, added 2026-07) ──
# These sites use a structured Spanish label format instead of the free-text
# style above: "Altitud: 1718 msnm  Altura: 10,40 m  # individuos: 1  Fotos:
# 9767 - 9761", with comma decimals (European style), occasional cm units,
# height ranges, and multiple individuals per point ("Otra"/"Otra (misma
# rama):" markers, or just a bare repeat of the labels). Detected separately
# from the free-text formats above so existing sites are unaffected.

_LABELED_FORMAT_RE = re.compile(r'\b(altura|forofito|individuos?)\b', re.I)


def _is_labeled_format(desc):
    return bool(_LABELED_FORMAT_RE.search(desc))


def _to_float(numstr):
    """European decimal comma -> float."""
    return float(numstr.replace(',', '.'))


# One token per relevant field, in the order they can appear in the collapsed
# (newline -> space) description string. Token type is whichever named group
# matched.
_LABELED_TOKEN_RE = re.compile(
    r'(?P<altitud>(?:altitud\s*:?\s*(?P<altitud_val>\d+(?:[.,]\d+)?)(?:\s*msnm)?)'
    r'|(?P<altitud_val2>\d+(?:[.,]\d+)?)\s*msnm)'
    r'|(?P<altura>(?:altura|forofito)\s*:?\s*'
    r'(?P<h1>\d+(?:[.,]\d+)?)\s*(?P<u1>cm|m)?'
    r'(?:\s*[-–]\s*(?P<h2>\d+(?:[.,]\d+)?)\s*(?P<u2>cm|m)?)?)'
    r'|(?P<individuos>(?:#\s*individuos?\s*#?\s*:?\s*(?P<n1>\d+))'
    r'|(?:individuos?\s*#\s*:?\s*(?P<n2>\d+))'
    # "N individuos" (count before the label, e.g. LaElenita's "2 individuos")
    # -- but not when a preceding unrelated number (e.g. a photo ID) happens
    # to sit right before a label that itself states its own count right
    # after, e.g. "...9539 Individuos #: 1" (count is 1, not 9539).
    r'|(?:(?P<n3>\d+)\s*individuos?\b(?!\s*#?\s*:?\s*\d)))',
    re.I
)


def parse_labeled_description(desc):
    """
    Parses the structured "Altitud:/Altura:/# individuos:" format via an
    ordered token stream (elevation once at the top, then alternating
    height/count pairs) rather than line-splitting, since geojson-to-csv.py
    collapses all newlines to spaces before this ever runs. A height with no
    explicit count that's followed by another height (not a count) before the
    string ends defaults to count=1; a count with no height of its own (e.g.
    "Otra (misma rama): # individuos: 2", extra individuals on the same perch
    with no new measurement) reuses the most recently seen height.
    """
    result = {'elevation_m': None, 'genus': None, 'canopy_m': None, 'individuals': []}

    last_height = None
    last_note = None
    pending = False  # True if last_height was set by an Altura not yet consumed by a count

    for m in _LABELED_TOKEN_RE.finditer(desc):
        if m.group('altitud'):
            if result['elevation_m'] is None:
                val = m.group('altitud_val') or m.group('altitud_val2')
                result['elevation_m'] = _to_float(val)
        elif m.group('altura'):
            h1 = _to_float(m.group('h1'))
            u1 = (m.group('u1') or 'm').lower()
            if m.group('h2'):
                h2 = _to_float(m.group('h2'))
                u2 = (m.group('u2') or u1).lower()
                if u1 == 'cm':
                    h1 /= 100
                if u2 == 'cm':
                    h2 /= 100
                last_height = round((h1 + h2) / 2, 3)
                last_note = f"range {h1}-{h2}m"
            else:
                last_height = h1 / 100 if u1 == 'cm' else h1
                last_note = None
            pending = True
        elif m.group('individuos'):
            n = int(m.group('n1') or m.group('n2') or m.group('n3'))
            result['individuals'].append(
                {'count': n, 'height_m': last_height, 'photo': None, 'note': last_note})
            pending = False

    if pending:
        result['individuals'].append(
            {'count': 1, 'height_m': last_height, 'photo': None, 'note': last_note})
...
def parse_description(desc):
    ...
    if _is_labeled_format(desc):
        return parse_labeled_description(desc)
...
def convert(input_csv, output_csv, source, site):
    ...
    HEIGHT_SANITY_CEILING = 20.0  # m; tallest measured canopy in the project so far is ~17m
    suspicious = df_out[df_out['Height_m'] > HEIGHT_SANITY_CEILING]
    if not suspicious.empty:
        print(f"\n  WARNING: {len(suspicious)} row(s) have Height_m > "
              f"{HEIGHT_SANITY_CEILING}m -- check these against the source photos/notes, "
              f"likely a data-entry issue (e.g. missing decimal separator):")
        for _, r in suspicious.iterrows():
            print(f"    lat={r['lat']}, lon={r['lon']}, Height_m={r['Height_m']}, "
